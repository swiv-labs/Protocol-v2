...
pub mod program {
    ...
    pub struct SwivPrivacy;
    #[automatically_derived]
    impl ::core::clone::Clone for SwivPrivacy {
        #[inline]
        fn clone(&self) -> SwivPrivacy {
            SwivPrivacy
        }
    }
    impl anchor_lang::Id for SwivPrivacy {
        fn id() -> Pubkey {
            ID
        }
    }
...
fn dispatch<'info>(
    ...
    if data.starts_with(instruction::ProcessUndelegation::DISCRIMINATOR) {
        return __private::__global::process_undelegation(
            program_id,
            accounts,
            &data[instruction::ProcessUndelegation::DISCRIMINATOR.len()..],
        );
    }
...
mod __private {
                        ...
                        anchor_lang::error::Error::from(anchor_lang::error::AnchorError {
                                ...
                                error_origin: Some(
                                    anchor_lang::error::ErrorOrigin::Source(anchor_lang::error::Source {
                                        filename: "programs/swiv_privacy/src/lib.rs",
                                        line: 7u32,
                                    }),
                                ),
    ...
    pub mod __global {
        use super::*;
        #[inline(never)]
        pub fn process_undelegation<'info>(
            __program_id: &Pubkey,
            __accounts: &'info [AccountInfo<'info>],
            __ix_data: &[u8],
        ) -> anchor_lang::Result<()> {
            ::solana_msg::sol_log("Instruction: ProcessUndelegation");
            let ix = instruction::ProcessUndelegation::deserialize(&mut &__ix_data[..])
                .map_err(|_| {
                    anchor_lang::error::ErrorCode::InstructionDidNotDeserialize
                })?;
            let instruction::ProcessUndelegation { account_seeds } = ix;
            let mut __bumps = <InitializeAfterUndelegation as anchor_lang::Bumps>::Bumps::default();
            let mut __reallocs = std::collections::BTreeSet::new();
            let mut __remaining_accounts: &[AccountInfo] = __accounts;
            let mut __accounts = InitializeAfterUndelegation::try_accounts(
                __program_id,
            ...
            let result = swiv_privacy::process_undelegation(
...
pub mod instruction {
    ...
    impl borsh::ser::BorshSerialize for ProcessUndelegation
    where
        Vec<Vec<u8>>: borsh::ser::BorshSerialize,
    {
        fn serialize<W: borsh::maybestd::io::Write>(
            ...
            borsh::BorshSerialize::serialize(&self.account_seeds, writer)?;
            Ok(())
        }
    }
    impl borsh::de::BorshDeserialize for ProcessUndelegation
    where
        Vec<Vec<u8>>: borsh::BorshDeserialize,
    {
        fn deserialize_reader<R: borsh::maybestd::io::Read>(
            ...
            Ok(Self {
                account_seeds: borsh::BorshDeserialize::deserialize_reader(reader)?,
            })
        }
    }
    impl anchor_lang::Discriminator for ProcessUndelegation {
        const DISCRIMINATOR: &'static [u8] = &[196, 28, 41, 206, 48, 37, 51, 167];
    }
    impl anchor_lang::InstructionData for ProcessUndelegation {}
    impl anchor_lang::Owner for ProcessUndelegation {
        fn owner() -> Pubkey {
            ID
        }
    }
...
pub mod accounts {
    pub use crate::__client_accounts_initialize_after_undelegation::*;
}
